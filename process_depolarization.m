...
for color_ch = 1:3
    ...
    NIR_G.ptt = {};
    for i_data=1:length(data_all)
        nir_data = data_all{i_data}.trace1(color_ch,:);
        g_data   = data_all{i_data}.trace3(color_ch,:);
        sid = data_all{i_data}.sid;
        if sid<11
            continue;
        end
...
%%
reg_bp(all_NIR_G{1}, 1, 0, {'SVR'}, 'Red');
%%
reg_bp(all_NIR_G{2}, 1, 0, {'SVR'}, 'Green');
%%
reg_bp(all_NIR_G{3}, 1, 0, {'SVR'}, 'Blue');
%%
...
% title('Periodogram')
%%
function [pd_bp, gt_bp]=reg_bp(NIR_G, channel_norms, same_segs, reg_mdls, title_name)
